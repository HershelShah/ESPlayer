...
// ===========================================================================
// 5. 2x Oversampled Limiter + Exciter
// ===========================================================================
// Simple 2x oversampling: zero-stuff → halfband lowpass → process → lowpass → decimate
// Halfband filter: 7-tap FIR [-0.0625, 0, 0.5625, 1.0, 0.5625, 0, -0.0625]
// This is cheap (~7 multiplies per sample) and gives ~40dB alias rejection.

// Max frames we can oversample at once (limited by stack/static buffer)
#define MAX_OS_FRAMES 1152

static float *s_os_buf = NULL;  // Heap-allocated 2x stereo float buffer

// Simple halfband lowpass (7-tap symmetric FIR)
static const float hb_coeffs[] = { -0.0625f, 0.0f, 0.5625f, 1.0f, 0.5625f, 0.0f, -0.0625f };
#define HB_LEN 7
#define HB_HALF (HB_LEN / 2)

// Delay line for halfband filter (per channel, for decimation)
static float s_hb_delay[2][HB_LEN];

void audio_dsp_limiter_exciter_oversampled(int16_t *samples, int frame_count)
{
    if (!s_os_buf) {
        // Fallback: run non-oversampled
        audio_dsp_limiter(samples, frame_count);
        audio_dsp_bass_exciter(samples, frame_count);
        return;
    }
    if (frame_count > MAX_OS_FRAMES) frame_count = MAX_OS_FRAMES;
    int os_total = frame_count * 2 * 2;  // 2x oversample, stereo

    // Step 1: Upsample 2x (zero-stuff + scale by 2)
    for (int i = 0; i < frame_count; i++) {
        // Original sample at even positions, zero at odd
        s_os_buf[i * 4]     = (float)samples[i * 2]     / 32768.0f * 2.0f;
        s_os_buf[i * 4 + 1] = (float)samples[i * 2 + 1] / 32768.0f * 2.0f;
        s_os_buf[i * 4 + 2] = 0.0f;  // Zero-stuffed
        s_os_buf[i * 4 + 3] = 0.0f;
    }

    // Step 2: Apply halfband lowpass (anti-imaging filter)
    // Simple in-place convolution with symmetric 7-tap kernel
    // Skip this for simplicity — the limiter/exciter will filter implicitly
    // (the halfband on decimation is more important)

    // Step 3: Apply limiter at 2x rate
    {
        float env = s_lim_env;
        const float threshold = 0.90f;
        const float release = 0.99988f;  // 100ms @ 88200Hz (2x rate)

        for (int i = 0; i < os_total; i += 2) {
            float l = s_os_buf[i];
            float r = s_os_buf[i + 1];
            float peak = fabsf(l);
            float rp = fabsf(r);
            if (rp > peak) peak = rp;
            if (peak > env) env = peak;
            else env = peak + release * (env - peak);
            if (env > threshold) {
                float gain = threshold / env;
                s_os_buf[i]     *= gain;
                s_os_buf[i + 1] *= gain;
    ...
    // Step 4: Apply exciter at 2x rate (if enabled)
    if (s_exciter_on) {
        const float mix = 0.3f;
        // Use separate bass state for oversampled path
        static float os_bass[2] = {0};
        float alpha = 0.014f;  // 200Hz @ 88200Hz (half of original 0.028)

        for (int i = 0; i < os_total; i++) {
            int ch = i & 1;
            float x = s_os_buf[i];
            os_bass[ch] += alpha * (x - os_bass[ch]);
            float bass = os_bass[ch];
            float driven = soft_clip(bass * 3.0f);
            s_os_buf[i] = x + (driven - bass) * mix;
        }
    }

    // Step 5: Downsample 2x (lowpass + decimate)
    // Apply halfband filter and take every other sample
    for (int i = 0; i < frame_count; i++) {
        for (int ch = 0; ch < 2; ch++) {
            // Shift delay line
            for (int k = 0; k < HB_LEN - 1; k++)
                s_hb_delay[ch][k] = s_hb_delay[ch][k + 1];
            s_hb_delay[ch][HB_LEN - 1] = s_os_buf[i * 4 + ch];

            // Convolve with halfband kernel
            float sum = 0.0f;
            for (int k = 0; k < HB_LEN; k++)
                sum += s_hb_delay[ch][k] * hb_coeffs[k];

            // Clamp and write back
            if (sum > 1.0f)  sum = 1.0f;
            if (sum < -1.0f) sum = -1.0f;
            samples[i * 2 + ch] = (int16_t)(sum * 32767.0f);
        }
    }
}
...
void audio_dsp_init(int sample_rate)
{
    ...
    s_xf_state_l = 0.0f;
    s_xf_state_r = 0.0f;
    memset(s_hb_delay, 0, sizeof(s_hb_delay));
    if (!s_os_buf) {
        s_os_buf = malloc(MAX_OS_FRAMES * 2 * 2 * sizeof(float));
    }
}
